...
def similarity_sorter(data_input, data_out):
    ...
    try:
        with open(data_input, "r", encoding="charmap") as dd:
            ...
            try:
                with open("group.txt", "r") as group_num:
                    old_data = group_num.readlines()
                    group_num = int(old_data[0])
                    sentence_index = int(old_data[1])
                        ...
                        if jaccard_similarity(line.split("-")[1].split(), line_2.split("-")[1].split()) > 0.4:
                            ...
                            if line_2.split() not in similarity_sentence_group and line_2.split("-")[1] not in all_sentence:
                                ...
                                if samo == 0:
                                    ...
                                    levenshtein_.append(pylev.levenshtein(line.split(), line_2.split()))
                                    euclidean_.append(euclidean_distance(nlp(line).vector, nlp(line_2).vector))
                    try:
                        if len(similarity_sentence_group) >= 2:
                            with open(data_out, "a", encoding="charmap") as d:
                                for num in range(len(similarity_sentence_group)):
                                    d.write(
                                        f"{sentence_index} - {group} - {numpy.round(jaccard_[num], 3)} - {numpy.round(cosine_[num], 3)} - {levenshtein_[num]} -"
                                        f" {euclidean_[num]} - {similarity_sentence_group[num]}")
                                    sentence_index += 1
                            group += 1
                            group_starting_num += 1
                            print(group)
                            try:
                                with open("group.txt", "w") as group_file:
...
def create_table_with_sentence(input_, output_):
    with open(input_, "r", encoding="charmap") as d:
        ...
        db = pandas.DataFrame({
            ...
            "email_index": [data[0].split("-")[6]],
            "jaccard_similarity": [data[0].split("-")[2]],
            "cosine_similarity": [data[0].split("-")[3]],
            "levenshtein_distance":[data[0].split("-")[4]],
            "euclidean_distance":[data[0].split("-")[5]],
            "sentence": [
        ...
        writer = pandas.ExcelWriter(output_, engine='xlsxwriter')
        db = db[
            ["similarity_group", "sentence_index", "email_index", "jaccard_similarity", "cosine_similarity", "levenshtein_distance", "euclidean_distance", "sentence"]
        ]
        num = 0
        for sentence in data:
            if sentence != data[0]:
                new_db = pandas.DataFrame({
                    ...
                    "email_index": [sentence.split("-")[6]],
                    "jaccard_similarity": [sentence.split("-")[2]],
                    "cosine_similarity": [sentence.split("-")[3]],
                    "levenshtein_distance":[sentence.split("-")[4]],
                    "euclidean_distance":[sentence.split("-")[5]],
                    "sentence": [
...
def sentence_with_email_index(input_file):
    ...
    with open(input_file, "r", encoding="charmap") as d:
        data = d.readlines()
        with open("sentence_with_email_index1.txt", "w", encoding="charmap" ) as dd:
