...
def create_from_to_table(emails, output):
    db = pandas.DataFrame({"email_index": [for_to(emails[0])[3].replace("Sent:", "").replace(",", "\n").strip()],
                           ...
                           "date_time": [dateparser.parse(for_to(emails[0])[2].replace("Sent:", "").replace(",", "\n").strip())],
                           })
    ...
    for email in emails:
        if for_to(email)[1]:
            ...
            new_db = pandas.DataFrame({
                ...
                "from": [for_to(email)[0].replace("From: ", "").replace("  ","").strip()],
                "to": [for_to(email)[1].replace("To:", "").replace(";", "\n").strip()],
                "date_time": [dateparser.parse(for_to(email)[2].replace("Sent:", "").replace(",", "\n").strip())],
            })
                    ...
                    if len(r) >= 2:
                        with open(data_out, "a") as d:
                            for sas in range(len(r)):
                                d.write(f"{group} - {numpy.round(jaccard_[sas], 3)} - {numpy.round(cosine_[sas], 3)} - {r[sas]}")
                        group += 1
                #         q = group
                except:
                    raise Exception
...
def create_table_with_sentence(input_, output_):
    with open(input_, "r") as d:
        ...
        db = db[["email_index", "similarity_group", "JaccardSimilarity", "CosineSimilarity", "sentence"]]
