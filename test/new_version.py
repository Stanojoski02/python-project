            ...
            if line != data_lines[0]:
                ...
                if from_to not in from_to_to_from and to_from not in from_to_to_from:
                    for line_2 in data_lines:
                        try:
                            if line.split('",')[1] in line_2 and line.split('",')[2] in line_2:
                                try:
                    ...
                    if from_to_sentence not in all_emails and from_to not in from_to_to_from and to_from\
                        ...
                        for i in from_to_sentence:
                            for j in from_to_sentence:
                                if jaccard_similarity(i[1].split(), j[1].split()) > 0.2  and j[1] not in senn:
                                    doc = nlp(str(j))
                                    PROPN = 0
                                    VERB = 0
                                    ADJ = 0
                                    for token in doc:
                                        if token.pos_ == "PROPN":
                                            ...
                                            ADJ += 1
                                    print(j[0] + " " + j[1] + f" PROPN: {PROPN} VERB: {VERB} ADJ: {ADJ}\n")
                                    with open(output_, "a", encoding="charmap") as n:
                                        n.write(str(email) + "," + str(jaccard_similarity(i[1].split(), j[1].split())) + "," + j[0] + "," + j[1] + "," + f" PROPN: {PROPN}, VERB: {VERB}, ADJ: {ADJ}" + "\n")
                                        senn.append(j[1])
