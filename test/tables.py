...
# new_sentance.txt
# pandas_table_with_sentence.xlsx
def sentence_table(input_,output_):
    with open(input_, "r") as d:
        data = d.readlines()
        db = pandas.DataFrame({
        ...
        for sentence in data:
            ...
            new_db = new_db[["email_index", "similarity_group", "similarity", "sentence"]]
            db = pandas.concat([db, new_db], ignore_index=True, axis=0)
        db.to_excel(writer, sheet_name='Sheet1', startrow=1, header=False, index=False)
        workbook = writer.book
        worksheet = writer.sheets['Sheet1']
...
# email_sentances_with_index()

# sentence_table('new_sentance.txt', 'pandas_table_with_sentence.xlsx')
sentence_table("sss.txt", "sss.xlsx")

# input_file ="sentance.txt"
# sorted_file = "new_sentance.txt"

# sorter("sentance.txt", "sss.txt")
