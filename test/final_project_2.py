...
def write_sentences_in_excel(input_, output_, file_num):
    ...
    with open(input_, "r", encoding="charmap") as d:
        ...
        try:
            ...
            db = pandas.DataFrame({
                ...
                "email_id": ["EMID" + str(sentences[0].split(",")[0].replace('"', ""))],
                "from(email index)": [
                    emails.index(
            ...
            try:
                if sentences[0] != sentence:
                    ...
                    new_db = pandas.DataFrame({
                        ...
                        "email_id": ["EMID"+ str(sentence.split(",")[0].replace('"', ""))],
                        "from(email index)": [
        ...
        try:
            ...
            new_writer = pandas.ExcelWriter(f"tbl_email_address_{file_num}.xlsx", engine='xlsxwriter')
            db_2 = db_2[
                ["ID", "email"]
            ]
            num = 0
        ...
        try:
            new_writer_1 = pandas.ExcelWriter(f"tbl_email_{file_num}.xlsx", engine='xlsxwriter')
            db_3 = db_3[
